...
async def register_handlers(dp: Dispatcher, bot_token):
    ...
    async def cmd_start(message: types.Message, state: FSMContext):
        ...
        welcome_message = ("️Уважаемый клиент приветствуем в ARAGON SHOP\n"
                           "Ознакомится с нашими правилами и ассотритемтом и акциями вы можете по данной ссылке  https://telegra.ph/-05-07-2551\n"
        ...
        if (match):
            ...
            if (cities):
                ...
                for index, (city_name, city_forkey) in enumerate(cities):
                    response_message += f"🚩 <i>{city_name}</i>\n<i>Выбрать</i> 👉 /order_{city_forkey}_{product_price_id}_{product_name_id}\n"
    ...
    async def order_payment_method_selectio1n(message: types.Message, state: FSMContext):
        ...
        if match:
            ...
            if 'card' in active_payment_types:
                response_message += f"💰 <i>Ручная оплата</i> 👉 /order_{city_forkey}_10_5_{product_id}\n\n"
                response_message += f"💰 <i>Оплата картой банка</i> 👉 /order_{city_forkey}_53_5_{product_id}\n\n"
                keyboard.add(types.KeyboardButton(f"Ручная оплата order_{city_forkey}_10_5_{product_id}"))
                keyboard.add(types.KeyboardButton(f"Оплата картой банка order_{city_forkey}_53_5_{product_id}"))
    ...
    async def balance_payment(message: types.Message, state: FSMContext):
        match = re.match(r'.*order_(.*)_35_5_(\d+)', message.text)
        if match:
            city_forkey = match.group(1)
        ...
        if match:
            ...
            if product_detail:
                ...
                response_message = (f"<b>💰 Вы заказали</b>\n"
                                    ...
                                    f"Номер заказа: {order_number}.\n"
                                    f"➖➖➖➖➖➖➖➖➖➖➖\n\n"
                                    f"Скопируйте и напишите оператору @DeadiOper\n"
                                    f"  текст данного сообщения для уточнения <b>НОМЕРА КАРТЫ!!!)</b>.Для получения Реквизитов -  по поводу оплат писать на данный Юзер @DeadiOper\n\n"
                                    f"Для получения Реквизитов -  по поводу оплат писать на данный Юзер @DeadiOper\n\n"
                                    ...
                                    f"3. дату и <b>ВРЕМЯ</b> платежа, а так же номер с которого оплачивали\n\n"
                                    f"➖➖➖➖➖➖➖➖➖➖➖\n"
                                    f"✔️ Проверить оплату\n"
                                    f"Жми 👉 /order_check\n\n"
                                    f"🚫 Отменить заказ\n"
    ...
    async def help_handler(message: types.Message):
        help_message = "❤️❤️Уважаемый клиент, рады приветствовать вас в нашем магазине! ❤️❤️❤️\n-Если у вас возникли трудности с поднятием клада или есть какие то вопросы по поводу кладов, обращайтесь к нашему оператору ARAGON SHOP (https://t.me/ARAGON_HELP).\n\n-Если вы хотите узнать подробнее о акциях и попасть в чаты то вам поможет наша команда админов чатов.\n\n-Если у вас есть жалобы или пожелания, то вам к Администрации магазина.\n\nТак же все возможные акции, игры, конкурсы вы можете узнать в разделе АКТИВНЫЕ АКЦИЯ\n❤️❤️❤️И МНОГО ДРУГИХ АКТУАЛЬНЫХ КОНТАКТОВ У НАС В ИНФО КАНАЛЕ \n (https://t.me/+kmmTqdpSO8EXhlOTc0)ИЛИ НА САЙТЕ (https://arahoron.skin/) МАГАЗИНА  ❤️❤️❤️"
    ...
    async def manual_payment_initiation(message: types.Message, state: FSMContext):
        ...
        response_message = (
            ...
            f"Номер заказа: {order_number}.\n\n"
                                    ...
                                    f"  текст данного сообщения для уточнения <b>НОМЕРА КАРТЫ!!!)</b>.Для получения Реквизитов -  по поводу оплат писать на данный Юзер @DeadiOper\n\n"
                                    f"Для получения Реквизитов -  по поводу оплат писать на данный Юзер @DeadiOper\n\n"
                                    f"Переведите на КАРТУ сумму <b>{rub_amount_fee} руб.</b>\n"
                                    f"В комментарии к платежу ничего указывать не нужно.\n"
                                    f"<b>ВНИМАНИЕ!</b> После оплаты сообщите оператору:\n"
                                    f"1. номер заказа <b>{order_number}</b>\n"
                                    f"2. <b>ОПЛАЧЕННУЮ СУММУ</b>\n"
